...
async fn main() -> eyre::Result<()> {
    ...
    let party_1_pg_client =
        PostgresClient::new(&args.db_url_party1, &args.schema_name_party1).await?;
    let party_2_pg_client =
        PostgresClient::new(&args.db_url_party2, &args.schema_name_party1).await?;
    let party_3_pg_client =
        PostgresClient::new(&args.db_url_party3, &args.schema_name_party1).await?;
