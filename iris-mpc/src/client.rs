...
pub async fn run_client(opts: Opt) -> eyre::Result<()> {
    ...
    let aws_clients = AwsClients::new_from_aws_config(shared_config, "local").await?;
    ...
    let requests_sns_client: Arc<Client> = Arc::new(aws_clients.sns_client);
    ...
    let recv_thread = spawn(async move {
        ...
        while counter < n_queries * 3 {
            // Receive responses
            let msg = aws_clients.sqs_client
                .receive_message()
