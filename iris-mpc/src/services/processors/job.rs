...
pub async fn process_job_result(
    ...
    let identity_match_results_by_target = request_types
        .iter()
        .enumerate()
        .filter(|(_, request_type)| {
            matches!(request_type.as_str(), IDENTITY_MATCH_CHECK_MESSAGE_TYPE)
        })
        .map(|(i, _)| {
            let request_id = request_ids[i].clone();
            let response_target = request_id_to_target
                .get(&request_id)
                .cloned()
            ...
            Ok((response_target.clone(), result_string))
        })
    ...
    let n_reset_check = identity_match_results_by_target
        .get("reset_check")
        .unwrap_or(&Vec::new())
        .len();
    let n_recovery_check = identity_match_results_by_target
        .get("recovery_check")
        .unwrap_or(&Vec::new())
        .len();
    ...
    .await?;
    let reset_check_results = identity_match_results_by_target
        .get("reset_check")
        .unwrap_or(&Vec::new())
        .clone();
    send_results_to_sns(
    ...
    let recovery_check_results = identity_match_results_by_target
        .get("recovery_check")
        .unwrap_or(&Vec::new())
        .clone();
    send_results_to_sns(
