...
use iris_mpc_common::helpers::smpc_request::{
    IdentityMatchCheckRequest, IDENTITY_DELETION_MESSAGE_TYPE, IDENTITY_MATCH_CHECK_MESSAGE_TYPE,
    REAUTH_MESSAGE_TYPE, RECOVERY_CHECK_MESSAGE_TARGET, RESET_CHECK_MESSAGE_TARGET,
...
pub async fn send_last_modifications_to_sns(
    ...
    for modification in &last_modifications {
        ...
        match modification.request_type.as_str() {
            ...
            // Backwards compatibility with old reset check message type
            RESET_CHECK_MESSAGE_TARGET => {
                reset_check_messages.push(body);
            }
            IDENTITY_MATCH_CHECK_MESSAGE_TYPE => {
                // deserialize the body into an IdentityMatchCheckRequest
                let identity_match_check_request: IdentityMatchCheckRequest =
                    serde_json::from_str(&body).unwrap();
                // Define
                match identity_match_check_request.response_target.as_str() {
                    RESET_CHECK_MESSAGE_TARGET => {
                        reset_check_messages.push(body);
                    }
                    ...
                    _ => {
                        tracing::error!(
                            ...
                            identity_match_check_request.response_target
                        );
                    }
                }
            }
