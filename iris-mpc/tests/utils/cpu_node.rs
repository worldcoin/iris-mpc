...
impl DbStores {
    ...
    /// `bincode::serialize_into(writer, &graph)` where `graph: BothEyes<GraphMem<SerialId>>`.
    ...
    pub async fn compute_reference_hash(&self) -> eyre::Result<[u8; 32]> {
        ...
        let mut graph_pair: [GraphMem<IrisSerialId>; 2] = [GraphMem::new(), GraphMem::new()];
        for row in &rows {
...
impl CpuNode {
    ...
    /// 3. Replay mutations into `[GraphMem<IrisSerialId>; 2]` (left + right eye).
    ...
    async fn make_checkpoint_inner(
        ...
        let mut left_graph = GraphMem::<IrisSerialId>::new();
        let mut right_graph = GraphMem::<IrisSerialId>::new();
        ...
        let bytes: Vec<u8> = match &format {
            GraphFormat::Current => {
                ...
                let graph_pair: [GraphMem<IrisSerialId>; 2] = [left_graph, right_graph];
                                ...
                                links: layer
                                    ...
                                    .map(|(k, vs)| {
                                        (
                                            vec_id(k),
                                            graph_v3::EdgeIds(
                                                vs.neighbors.iter().map(&vec_id).collect(),
                                            ),
                                        )
                                    })
                                    .collect(),
