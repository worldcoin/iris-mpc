...
use iris_mpc_common::{
    helpers::{
        ...
        sha256::sha256_bytes,
        smpc_request::{REAUTH_MESSAGE_TYPE, RECOVERY_CHECK_MESSAGE_TYPE, RESET_CHECK_MESSAGE_TYPE, UNIQUENESS_MESSAGE_TYPE},
    },
...
impl ServerActor {
            ...
            let skip_lookback_requests: HashSet<usize> = batch
                ...
                .filter(|(_, req_type)| {
                    req_type.as_str() == REAUTH_MESSAGE_TYPE
                        || req_type.as_str() == RESET_CHECK_MESSAGE_TYPE || req_type.as_str() == RECOVERY_CHECK_MESSAGE_TYPE
                })
                .map(|(index, _)| index)
                .collect();
    ...
    fn compare_query_against_db_subset_and_self(
        ...
        let max_subset_size = db_subset_idx.iter().map(|x| x.len()).max().unwrap_or(0);

        for idx in (0..max_subset_size).step_by(DB_CHUNK_SIZE) {
            let start = idx;
            let end = idx + DB_CHUNK_SIZE;
            let db_subset_idx_chunk = db_subset_idx
                .iter()
                .map(|indices| {
                    let len = indices.len();
                    indices[start.min(len)..end.min(len)].to_vec()
                })
                .collect::<Vec<_>>();
            // We copied over a subset of the db, so we match against DB chunks of the given sizes
            let chunk_size = db_subset_idx_chunk
                .iter()
                .map(|x| x.len())
                .collect::<Vec<_>>();
