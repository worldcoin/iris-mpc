    ...
    if (idx < numElements)
    {
        ...
        for (int i = 0; i < 64; i++)
        {
            ...
            if (match_distances_counter_idx < max_bucket_distances)
            {
                ...
                // 2. dbIdx: to distinguish between different database entries, range [0, totalDbLen)]
                ...
                unsigned long long match_id = batch_id * (totalDbLen * (ALL_ROTATIONS * max_batch_size)) // 1.
                                              + (dbIdx + offset) * (ALL_ROTATIONS * max_batch_size) // 2.
                                              + queryIdx; // 3.
