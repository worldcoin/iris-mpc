...
mod conditionally_select_test {
    ...
    fn real_result(
        ...
        for (cond, code_1, mask_1, code_2, mask_2) in
            izip!(conds, code_input, mask_input, code_input_2, mask_input_2)
        {
            match cond {
                1 => {
                    code.push(code_1);
                ...
                _ => panic!("Invalid condition value: {}", cond),
            }
        }
        (code, mask)
    }
    ...
    fn testcase(
        mut party: Circuits,
        cond_share_a: Vec<u32>,
        cond_share_b: Vec<u32>,
    ...
    async fn test_cond_select() -> Result<()> {
        ...
        // Get inputs
        let conds = sample_conds(INPUTS_PER_GPU_SIZE * n_devices, &mut rng);
        ...
        let (cond_share_a, cond_share_b, cond_share_c) = rep_share_vec(&conds, &mut rng);
        ...
        let real_result = real_result(conds, code_dots, mask_dots, code2_dots, mask2_dots);
        tracing::info!("Random shared inputs generated!");

        let cond_share_a_ = cond_share_a.to_owned();
        let cond_share_b_ = cond_share_b.to_owned();
        let cond_share_c_ = cond_share_c.to_owned();
