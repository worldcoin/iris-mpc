...
impl SyncResult {
    ...
    pub fn compare_modifications(&self) -> (Vec<Modification>, Vec<Modification>) {
        ...
        for (&id, group_mods) in &grouped {
            ...
            let any_completed = group_mods.iter().any(|m| m.status == STATUS_COMPLETED);
            let all_in_progress = group_mods.iter().all(|m| m.status == STATUS_IN_PROGRESS);
            ...
            // If any node completed => unify to COMPLETED
                ...
                match local_copy {
                    ...
                    Some(local_m) => {
                        ...
                        if local_m.status != STATUS_COMPLETED || local_m.persisted != any_persisted
                        {
                            // We'll roll forward local_m
                            let mut roll_forward = local_m.clone();
                            roll_forward.status = STATUS_COMPLETED.to_string();
