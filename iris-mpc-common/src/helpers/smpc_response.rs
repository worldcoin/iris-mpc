...
pub struct UniquenessResult {
    /// Party id of the party generating the result.
    pub node_id: usize,
    /// Inserted iris serial id if codes are unique, empty otherwise.
    pub serial_id: Option<u32>,
    /// Whether request matched with existing iris code.
    pub is_match: bool,
    /// Unique identifier for the request, a UUID generated by the orb along
    /// with each request.
    pub signup_id: String,
    /// Serial ids of the final matched results, empty if `is_match`` is false.
    pub matched_serial_ids: Option<Vec<u32>>,
    /// Serial ids of the left matched partial results.
    pub matched_serial_ids_left: Option<Vec<u32>>,
    /// Serial ids of the right matched partial results.
    pub matched_serial_ids_right: Option<Vec<u32>>,
    /// Count of the right partial matches.
    pub partial_matches_count_right: Option<usize>,
    /// Count of the left partial matches.
    pub partial_matches_count_left: Option<usize>,
    /// The indices of the rotations that produced the partial matches for the
    /// left side. Each element is an array of rotation indices for the
    /// corresponding partial match.
    pub partial_match_rotation_indices_left: Option<Vec<Vec<i8>>>,
    /// The indices of the rotations that produced the partial matches for the
    /// right side. Each element is an array of rotation indices for the
    /// corresponding partial match.
    pub partial_match_rotation_indices_right: Option<Vec<Vec<i8>>>,
    /// Signup ids of intra-batch matches with the request.
    pub matched_batch_request_ids: Option<Vec<String>>,
    /// Whether a mirror attack was detected for this request.
    pub full_face_mirror_attack_detected: bool,
    /// Serial ids that matched during the mirror orientation run of the
    /// uniqueness check.
    pub full_face_mirror_matched_serial_ids: Option<Vec<u32>>,
    /// Serial ids that partially matched during the mirror orientation run of
    /// the uniqueness check for the left side.
    pub full_face_mirror_matched_serial_ids_left: Option<Vec<u32>>,
    /// Serial ids that partially matched during the mirror orientation run of
    /// the uniqueness check for the right side.
    pub full_face_mirror_matched_serial_ids_right: Option<Vec<u32>>,
    /// Count of the full face mirror left partial matches.
    pub full_face_mirror_partial_matches_count_left: Option<usize>,
    /// Count of the full face mirror right partial matches.
    pub full_face_mirror_partial_matches_count_right: Option<usize>,
    /// The error that occurred during the MPC uniqueness check.
    pub error: Option<bool>,
    /// The reason for an error that occurred during the MPC uniqueness check.
...
pub struct ReAuthResult {
    ...
    /// Party id of the party generating the result.
    pub node_id: usize,
    /// Serial id of the target iris to be matched.
    pub serial_id: u32,
    /// Whether the Reauth request matched its target under the configured
    /// OR/AND rule.
    pub success: bool,
    /// List of serial ids that were matched.
    pub matched_serial_ids: Vec<u32>,
    /// Whether the Reauth was performed using OR rule.
    pub or_rule_used: bool,
    /// Error occurred during the MPC reauth check.
    pub error: Option<bool>,
...
pub struct IdentityMatchCheckResult {
    ...
    /// Count of the left partial matches
    pub partial_matches_count_left: Option<usize>,
    /// Error occurred during the MPC identity match check.
    pub error: Option<bool>,
    /// Error reason, if an error occurred.
    pub error_reason: Option<String>,
}
