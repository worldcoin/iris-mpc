...
pub fn get_meminfo(fd: RawFd) -> Result<sk_meminfo> {
    let (r, ti) = getsockopt(fd, libc::SOL_SOCKET, libc::SO_MEMINFO);
    match r {
        0 => Ok(ti),
...
/// taken from /include/uapi/linux/sock_diag.h - the SK_MEMINFO enum
/// http://oldvger.kernel.org/~davem/skb_sk.html
/// https://www.man7.org/linux/man-pages/man8/ss.8.html
#[repr(C)]
#[derive(Debug, Default, Clone, Copy)]
pub struct sk_meminfo {
    /// The number of bytes from incoming packets. up to rcvbuf bytes
    pub rmem_alloc: u32,
    /// the total number of bytes of "receive packet memory" a socket may use
    pub rcvbuf: u32,
    /// The number of bytes from outgoing packets. up to sndbuf bytes
    pub wmem_alloc: u32,
    /// the total number of bytes of "send packet memory" a socket may use
    pub sndbuf: u32,
    /// the memory allocated by the socket as cache but not yet used for sending/receiving packets
    pub fwd_alloc: u32,
    /// payload queued for transmit but not yet sent to the NIC
    pub wmem_queued: u32,
    /// Amount of memory used for socket-option state (bytes).
    pub optmem: u32,
    /// the memory used for the sk backlog queue
    pub backlog: u32,
    /// cumulative packets dropped before they are demultiplexed into the socket
    pub drops: u32,
}
