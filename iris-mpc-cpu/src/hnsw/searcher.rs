...
impl HnswSearcher {
    ...
    async fn layer_search_batched_v2<V: VectorStore, N: Neighborhood<V>>(
        ...
        while !cur_unopened.is_empty() {
            ...
            metrics::histogram!("layer_search_visited_per_iteration").record(c_links.len() as f64);
    ...
    pub async fn insert_prepare<V: VectorStore>(
        ...
        self.insert_prepare_batch(store, graph, mutations).await
    }
        ...
        if !needs_compaction.is_empty() {
                ...
                izip!(&base_nodes, &layers, &neighborhoods, compacted_nbhds)
            {
                ...
                if !to_remove.is_empty() {
                    mutations.push(GraphMutation::Compact {
                        id: id.clone(),
                        layer: *layer,
                        to_remove,
                    });
...
mod tests {
    ...
    async fn test_insert_prepare_batch() -> Result<()> {
        ...
        // Verify the mutation is an InsertNode with the correct properties
        if let GraphMutation::InsertNode {
            id,
            layers,
            update_ep,
