...
#[instrument(level = "trace", target = "searcher::network", skip_all)]
async fn bit_inject_2round_party0<T: IntRing2k + NetworkInt>(
    session: &mut Session,
    input: VecShare<Bit>,
) -> Result<VecShare<T>, Error>
where
...
{
    let len = input.len();
    let mut res_a = Vec::with_capacity(len);
    let prf = &mut session.prf;

    for inp in input.into_iter() {
        // New random share (also generated by Sender)
        let masking_share = prf.gen_zero_share();
        let extended_bit = if inp.get_b().convert().convert() {
            RingElement::one()
        } else {
            RingElement::zero()
        };
        let z0 = masking_share * extended_bit;
        let y0 = masking_share + extended_bit - z0 - z0;
        res_a.push(y0);
    }
    ...
    .increment(1);
    // Send res_a to party 1
    network
        .send_next(T::new_network_vec(res_a.clone()).to_network())
        .await?;

    // Receive res_b from party 2
    let res_b = {
        let reply = network.receive_prev().await;
...
#[instrument(level = "trace", target = "searcher::network", skip_all)]
async fn bit_inject_2round_party1<T: IntRing2k + NetworkInt>(
    session: &mut Session,
    input: VecShare<Bit>,
) -> Result<VecShare<T>>
where
    Standard: Distribution<T>,
{
    ...
    for inp in input.into_iter() {
        ...
        let masking_share = prf.gen_zero_share();
        res_a.push(
            if inp.get_a().convert().convert() ^ inp.get_b().convert().convert() {
                masking_share + RingElement::one()
            } else {
                masking_share
            },
        );
    ...
    .increment(1);
    // Send res_a to party 2
    network
        .send_next(T::new_network_vec(res_a.clone()).to_network())
        .await?;

    // Receive res_b from party 0
    let res_b = {
        let reply = network.receive_prev().await;
        match NetworkValue::from_network(reply) {
            Ok(v) => T::into_vec(v),
...
#[instrument(level = "trace", target = "searcher::network", skip_all)]
async fn bit_inject_2round_party2<T: IntRing2k + NetworkInt>(
    session: &mut Session,
    input: VecShare<Bit>,
) -> Result<VecShare<T>, Error>
where
    Standard: Distribution<T>,
{
    ...
    let res_a = input
        ...
        .map(|(inp, b)| {
            let masking_share = prf.gen_zero_share();
            let y = if inp.a.convert().convert() {
                RingElement::one()
            } else {
                RingElement::zero()
            };
            let z2 = y * (*b + masking_share);
            masking_share - z2 - z2
        })
        .collect_vec();
...
/// Performs a bit injection protocol in 2 rounds.
/// The input is a vector of bit shares, the output is a vector of ring shares of type T.
///
/// The protocol works as follows:
/// - One party combines its two replicated shares (x0, x1) into a single share (x0 + x1) and provides this as a secret input to the protocol.
///   - This requires 1 instance of communication to re-establish replicated sharing.
/// - The parties then perform an arithmetic XOR (a+b-2ab) of the two inputs (x0 + x1) and (x2) to obtain a share of the output.
///   - However, using the "special multiplication" protocol, since x2 is known to two parties, the output can be computed by communicating two instances of communication in one round instead of the normal 3.
///   - For more details, see Section 3.2 of <https://eprint.iacr.org/2025/919.pdf>.
/// - Overall this is a 2-round protocol with 3 * sizeof(T) communication in total.
pub(crate) async fn bit_inject_2round<T: IntRing2k + NetworkInt>(
    session: &mut Session,
    input: VecShare<Bit>,
) -> Result<VecShare<T>, Error>
where
    Standard: Distribution<T>,
{
    let res = match session.own_role().index() {
        0 => bit_inject_2round_party0::<T>(session, input).await?,
        1 => bit_inject_2round_party1::<T>(session, input).await?,
        2 => bit_inject_2round_party2::<T>(session, input).await?,
        _ => {
            bail!("Cannot deal with roles outside of the set [0, 1, 2] in bit_inject")
        }
    };
    Ok(res)
}
...
pub(crate) async fn lift(session: &mut Session, shares: VecShare<u16>) -> Result<VecShare<u32>> {
    ...
    b1.extend(b2);
    let mut b = bit_inject_2round(session, b1).await?;
