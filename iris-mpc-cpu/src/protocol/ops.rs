...
use iris_mpc_common::{
    fast_metrics::FastHistogram, galois_engine::degree4::SHARE_OF_MAX_DISTANCE, vector_id::VectorId,
};
...
/// Assumes that the input shares are originally 16-bit and lifted to u32.
/// The vector ids are in plaintext, but only the 0-indexed part is used and propagated in secret shared form.
#[instrument(level = "trace", target = "searcher::network", skip_all)]
pub async fn conditionally_swap_distances_plain_ids(
    session: &mut Session,
    swap_bits: Vec<Share<Bit>>,
    list: &[(VectorId, DistanceShare<u32>)],
    indices: &[(usize, usize)],
    ...
    let mut encrypted_list = list
        .iter()
        .map(|(v, d)| {
            let shared_index = Share::from_const(v.index(), role);
    ...
    for ((bit, (idx1, idx2)), (first_d, second_d)) in swap_bits_u32.iter().zip(indices.iter()).zip(
        first_distances
            .into_iter()
            .zip(second_distances.into_iter()),
    ) {
        let mut not_bit = -bit;
        ...
        let first_id = bit * v1.index() + not_bit.clone() * v2.index();
        let second_id = bit * v2.index() + not_bit * v1.index();
...
pub async fn conditionally_swap_distances(
    ...
    let res_a: Vec<RingElement<u32>> = indices
        ...
        .flat_map(|((idx1, idx2), sb)| {
            ...
            let id = mul_share_a(d1.0.clone(), d2.0.clone(), sb);
            let code_dot_a = mul_share_a(d1.1.code_dot.clone(), d2.1.code_dot.clone(), sb);
            let mask_dot_a = mul_share_a(d1.1.mask_dot.clone(), d2.1.mask_dot.clone(), sb);
    ...
    // First tuples
    let swapped_distances = izip!(res_a.into_iter(), res_b.into_iter())
        // combine a and b part into shares
        .map(|(a, b)| Share::new(a, b))
        // combine the code and mask parts into DistanceShare
        .tuples()
        ...
        .map(|(res, (idx1, idx2))| {
            let id1 = &list[*idx1].0;
            let dist1 = &list[*idx1].1;
            let id2 = &list[*idx2].0;
            let dist2 = &list[*idx2].1;
            // first tuple element = c * (d1 - d2) + d2
            // second tuple element = d1 - c * (d1 - d2)
            let first_id = res.0.clone() + id2;
            let second_id = id1.clone() - res.0;
            let first_distance = DistanceShare {
                code_dot: res.1.code_dot.clone() + &dist2.code_dot,
                mask_dot: res.1.mask_dot.clone() + &dist2.mask_dot,
            };
            let second_distance = DistanceShare {
                code_dot: dist1.code_dot.clone() - res.1.code_dot,
                mask_dot: dist1.mask_dot.clone() - res.1.mask_dot,
            };
