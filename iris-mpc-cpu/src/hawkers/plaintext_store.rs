...
use serde_json::{self, Deserializer, to_writer};
...
impl PlaintextStore {
    ...
    pub fn read_ndjson_file(filename: &str, len: usize) -> io::Result<Self> {
        ...
        for json_pt in stream.take(len) {
            let json_pt = json_pt?;
        ...
        if vector.points.len() == len {
            Ok(vector)
        } else {
            Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!("File {} contains too few entries; number read: {}", filename, vector.points.len())
            ))
        }
    }

    pub fn write_ndjson_file(&self, filename: &str) -> std::io::Result<()> {
        // Serialize the objects to the file
        let file = File::create(filename)?;
        let mut writer = BufWriter::new(file);
        for pt in &self.points {
            let json_pt: Base64IrisCode = (&pt.data.0).into();
            to_writer(&mut writer, &json_pt)?;
