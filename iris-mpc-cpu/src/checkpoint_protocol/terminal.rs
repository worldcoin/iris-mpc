...
pub struct InstallAsServing {
    pub target: BothEyes<Arc<RwLock<GraphMem<SerialId>>>>,
}

impl InstallAsServing {
    pub fn new(target: BothEyes<Arc<RwLock<GraphMem<SerialId>>>>) -> Self {
        Self { target }
    }
}
...
mod tests {
    ...
    async fn install_as_serving_swaps_in_snapshot_graph() {
        ...
        let target: BothEyes<Arc<RwLock<GraphMem<SerialId>>>> = [
    ...
    fn consensus_hash_matches_upload_path_serialization() {
        ...
        let mut left = GraphMem::<SerialId>::new();
        let mut right = GraphMem::<SerialId>::new();
