...
async fn per_side(
    ...
    let results = izip!(chunks, sessions)
        ...
        .collect::<JoinAll<_>>()
        .await;
    // results of the process should be the minimum of the number of sessions and the missing_vector_ids*rotations
    tracing::info!("per_side_match_batch results: {}", results.len());
