...
use iris_mpc_cpu::{
    database_generators::{create_random_sharing, generate_iris_shares, ng_generate_iris_shares},
    hawkers::{
        aby3_store::create_ready_made_hawk_searcher,
...
fn bench_ready_made_hnsw(c: &mut Criterion) {
    ...
    for database_size in [1, 10, 100, 1000, 10000] {
        ...
        group.bench_function(BenchmarkId::new("big-hnsw-searches", database_size), |b| {
            b.to_async(&rt).iter_batched(
                || secret_searcher.clone(),
                |mut my_db| async move {
                    let mut rng = AesRng::seed_from_u64(0_u64);
