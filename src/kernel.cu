...
extern "C" __global__ void reconstructDistance(unsigned short* codes_result1, unsigned short* codes_result2, unsigned short* codes_result3, unsigned short* masks_result1, unsigned short* masks_result2, unsigned short* masks_result3, unsigned float* output, unsigned short p) {
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < numElements) {
