use super::{
    galois::{GaloisRingElement, ShamirGaloisRingShare},
...
mod tests {
    ...
    fn galois_dot_trick() {
        for _ in 0..10 {
            let iris_db = super::IrisCodeArray::random_rng(&mut thread_rng());
            let iris_query = super::IrisCodeArray::random_rng(&mut thread_rng());
            let shares = super::GaloisRingIrisCodeShare::encode_iris_code(&iris_db);
            let query_shares = super::GaloisRingIrisCodeShare::encode_iris_code_query(&iris_query);
            let mut dot = [0; 3];
            for i in 0..3 {
