...
pub(super) async fn download_public_keyset(base_url: &str) -> Result<PublicKeyset> {
    async fn download_key(party_idx: usize, base_url: &str) -> Result<PublicKey, PublicKeyError> {
        ...
        let pbk = PublicKey::from_slice(&pbk_bytes).ok_or_else(|| {
    ...
    let (key0, key1, key2) = tokio::join!(
        download_key(PARTY_IDX_0, base_url),
        download_key(PARTY_IDX_1, base_url),
        download_key(PARTY_IDX_2, base_url)
    );
