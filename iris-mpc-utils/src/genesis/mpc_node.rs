...
pub struct MpcNodes {
    nodes: [Arc<MpcNode>; N_PARTIES],
}

impl MpcNodes {
    pub async fn new(net_config: &NetConfig) -> Self {
        Self {
            nodes: [
                Arc::new(MpcNode::new(net_config[0].clone()).await),
                Arc::new(MpcNode::new(net_config[1].clone()).await),
                Arc::new(MpcNode::new(net_config[2].clone()).await),
            ],
        }
    }
...
impl IntoIterator for MpcNodes {
    ...
    type IntoIter = std::array::IntoIter<Arc<MpcNode>, N_PARTIES>;
...
impl MpcNode {
    ...
    pub async fn persist_modification(&self, id: i64) -> Result<()> {
        let mut tx = self.gpu_stores.iris.tx().await?;
        db_ops1::persist_modification(&mut tx, id).await?;
        tx.commit().await?;
...
impl DbAssertions {
    ...
    pub async fn assert(&self, stores: &DbStores) -> Result<()> {
        ...
        if let Some(vector_ids) = &self.vector_ids {
            let store_vector_ids = db_ops1::get_iris_vector_ids(&stores.iris).await?;
