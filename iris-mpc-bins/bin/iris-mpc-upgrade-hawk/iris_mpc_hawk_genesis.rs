...
fn main() -> Result<()> {
    // Pin tokio threads to node 1 (upper half of cores)
    let mut core_ids = core_affinity::get_core_ids().unwrap();
    core_ids.sort();
    let node1_cores: Vec<CoreId> = core_ids[core_ids.len() / 2..].to_vec();
    let num_tokio_threads = node1_cores.len();
    static TOKIO_IDX: AtomicUsize = AtomicUsize::new(0);

    let runtime = tokio::runtime::Builder::new_multi_thread()
        .worker_threads(num_tokio_threads)
        .on_thread_start(move || {
            let i = TOKIO_IDX.fetch_add(1, Ordering::Relaxed) % num_tokio_threads;
            let _ = core_affinity::set_for_current(node1_cores[i]);
        })
        .enable_all()
        .build()
        .unwrap();
